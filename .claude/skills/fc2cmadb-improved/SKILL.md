...
7. **无图卡片 cover.jpg 兜底必须懒加载**：站点卡片 `<img>` 在“未登录”或“真实缩略图失效
   ...
   `setupCoverFallback` 曾一次性对整页所有无图卡片请求 baihuse 明细页（未登录时整页 30 张
   全是 no-image），瞬间打出大量请求，把详情页预览（同样依赖 baihuse）拖垮成“没有预览”。
   现已改为 `IntersectionObserver` 懒加载（卡片进入视口才请求），并保留 `error` 兜底。
   改动此项时不要退回“整页一次性请求”。
